...
| `02b_stage1_preamble_bakeoff.ipynb` (`build_02b.py`) | `02b-stage1-preamble-bakeoff` | campaign 1 bake-off on the 100-preamble pilot (`pilot_preamble_100.json`), codebook v2.0, prompt `q1-f2f23d89` (`PROMPT_CELL_Q1`: T5 = ground + frame + operator, no quotable markers), grammar from schema v2.0, `max_new_tokens=3072`; §7 adds ground/frame shares and the operator = rule(ground) consistency check; §8 α on operator, ground and frame on aligned claims; §9 paraphrase `q1b-`; writes to `stage1_preamble/` | built (16 Sep 2026); smoke test pending |
