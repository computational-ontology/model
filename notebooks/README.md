...
| `align_claims.py` (script, no Kaggle) | — | claim-level agreement on the offsets form: greedy span alignment (IoU ≥ `--iou`, default 0.5), Krippendorff α on operator, ground, frame; per-model operator = rule(ground) check; confusion table | done (16 Sep 2026) |
