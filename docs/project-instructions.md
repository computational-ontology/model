# Computational-Ontology Model — Project instructions (v0.4, "the books")
...
Changes since v0.3 (decisions D22-D23, 16 Sep 2026, after reading *Documentalidad* and the *Manifiesto* — see the project note `FERRARIS_LIBROS`): the bridge in C.4 is reformulated (↓ is the mirror image of the transcendental fallacy, not an instance of it); the vocabulary gains *natural* = *physical*, *foundation by the spirit / by the letter*, *ground* and *frame*; axiom 5 is grounded in *Manifiesto* p. 83 and gets a guardrail against the knowledge-power fallacy; C.4's open point on LLMs as emitters is closed with *Manifiesto* p. 78. Nothing else changes.
...
| **Naturalisation (↓)** | The operator that presents an inscribed act — or the amendable scheme behind it — as unamendable reality ("beliefs wearing the costume of plain facts"; ontological mimicry). In founding inscriptions: the act is made to rest on a *ground* presented as prior to and independent of any act. Ferraris's name for it: *foundation according to the spirit* (*Documentalidad*, pp. 353-355) | "lie", "disinformation", "false" |
| **Revelation (✦)** | The inverse operator: the act exposes its own act character — a named subject, a procedure, a mandate, a prior inscription. Ferraris: *foundation according to the letter* (ibid.); legitimacy "by appeal to other inscriptions held valid" (p. 333 n. 8) | "critique", "truth" |
| **Ground** | What a clause makes the founding act rest on: `nature`, `history`, `god`, `spirit`, `doctrine` (naturalising grounds), `act` (revealing ground) or `none` (codebook v2.0 §3, field `ground`) | "topic", "theme" |
| **Frame** | How the enunciator relates a clause to itself: `attitude`, `will`, `procedure`, `invocation`, `narrative`, `none` (codebook v2.0 §3, field `frame`). An attitude frame ("convinced that…") is a constative element inside a performative document (*Documentalidad*, pp. 365-367) and never changes the ground | "modality" |
| **Natural object** | Ferraris's own term (thesis 2, *Documentalidad* p. 423) for what this project's code labels `physical`; the two are synonyms here | — |
...
**Bridge to Ferraris (reformulated in v0.4, D23).** Ferraris's transcendental fallacy runs in one direction — what *is* made to depend on what we *know* (*Manifiesto*, pp. 29, 35; introductory study pp. 54-55) — and it is a fallacy about natural objects only: for social objects transcendentalism "has citizenship" (*Documentalidad*, p. 217; p. 220: "how much the ontology of social objects depends on epistemology"; thesis 4). The *naturalisation* operator (↓) of *Ideological Architectures* runs the other way: a social object, legitimately dependent on subjects, acts and inscriptions, is presented as if it were a natural, unamendable, subject-independent object. The two are mirror images of one collapse of the ontology/epistemology distinction — the transcendental fallacy epistemologises being; naturalisation ontologises a decision — and ↓ is therefore best described as **the naturalist fallacy of the social, executed by a social actor upon a public**. On the reception side it exploits the *acertar-aceptar* fallacy (*Manifiesto*, pp. 63-64): once the act is presented as fact, dissent looks like denial of fact and taking note is made to look like accepting. Its criterion is Hume's law as Ferraris draws it: inside social objects the ought is derived from the is "of their specific being, that is, their quality as social objects" (*Documentalidad*, p. 223) — the revealed ground (✦); from nature, God-as-fact, history-as-fact or the spirit of a people "no right and no duty can be obtained" (*Manifiesto*, p. 67) — the naturalised ground (↓). *Ontological mimicry* ("beliefs wearing the costume of plain facts", CPSS 2026) remains its computational name, and Table 1 remains the detection instrument: measuring how much of what a corpus presents as "fact" is a naturalised inscribed act. What ↓ never says is that the fact is false: past events are irrevocable (*Manifiesto*, pp. 69-71); ↓ labels the derivation of the act from them, not the event.

**E3 grounded and guarded (v0.4).** Social epistemology is performative — "in the social world new objects are always produced … a performative and not only constative operation" (*Manifiesto*, p. 83) — so the pipeline's outputs are new inscriptions, an increment of *realia* (pp. 80-81), and the model card says so. An act "between an instrument and a person" counts as a social act (p. 78): an LLM output is an inscribed act (emitter = model, date = run) with no claim about the model's intentionality (p. 88). Agreement among emitters is solidarity, not objectivity (pp. 97-99); objectivity for T5 comes only from anchoring in the hash-verified record through human annotation (D11). The reflexive register is Ferraris's "norm of prudence" (p. 93): the pipeline declares its interests and assumptions and does not discount its results — never the knowledge-power fallacy in its mechanical form (p. 94).
...
- LLMs as emitters: since the CPSS 2026 paper audits language models by perspective, this pipeline must be able to treat an LLM's output as an inscription (weak document, emitter = model, date = run) and apply the same ideological signature to it. **Closed in v0.4**: an act "between an instrument and a person" is a social act in Ferraris's own definition (*Manifiesto*, p. 78); `Stage1Record` already records emitter, date, prompt and decoder.
- Pilot corpus, stack and compute are decided in `plan.md` (D1–D3, §3, §5b).

---

## PART D — Self-audit of this document (v0.4)
...
- The bridge of v0.2-v0.3 ("↓ = the transcendental fallacy performed socially") was checked against the books on 16 Sep 2026 and found to run in the wrong direction; the v0.4 formulation (mirror image; naturalist fallacy of the social; Hume's law as criterion) is still a **proposal of this project**, now with page citations, not a published result. The `ground`/`frame` fields and the "foundation by the spirit / by the letter" gloss are the project's readings; Ferraris does not connect those pages to founding documents. *Il mondo esterno* pp. 19-20 remains to be checked.
